...
3. **首次啟動與 Gatekeeper 安全授權說明**：
   ...
   - **方法 A**：前往 Mac **「系統設定」➔「隱私權與安全性」**，向下滑動至安全性區塊，點擊 **「仍要打開 (Open Anyway)」**。
   - **方法 B（終端機指令）**：直接在終端機輸入：
     ```bash
...
- **💬 智慧長推文自動分段小幫手 (`⌘ + Shift + X` / `Alt + X`)**：長文自動斷行分段、PTT/黃金之島自動推噓協定適配、帳號ID字數安全精算、標點優先與極致塞滿雙排版策略，支援逐行手動或一鍵全自動連續推文。
